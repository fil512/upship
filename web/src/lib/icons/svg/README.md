...
| `engineers.svg` | Personnel who aid research and handle hazard repairs | Hard hat with safety goggles | #ffa726 (Orange) |
...
| `vp.svg` | Victory Points—the win condition | Five-pointed star | #4caf50 (Green) |
...
| `any.svg` | Wild symbol—works at any location | Five-pointed star (filled) | #c4a35a (Tan/Gold) |
...
| File | Game Meaning | Current Shape | Color |
|------|--------------|---------------|-------|
| `germany.svg` | Germany—rigid airship pioneers (Zeppelin) | Horizontal stripes | #000000, #DD0000, #FFCC00 |
| `britain.svg` | Britain—imperial route masters | Union Jack (simplified) | #012169, #ffffff, #C8102E |
| `usa.svg` | USA—helium monopoly holders | Stars and stripes (simplified) | #BF0A30, #ffffff, #002868 |
| `italy.svg` | Italy—semi-rigid specialists | Vertical stripes | #009246, #ffffff, #CE2B37 |
...
| File | Game Meaning | Current Shape | Color |
|------|--------------|---------------|-------|
| `lift.svg` | Buoyancy from gas cells—determines cargo capacity | Upward arrow with ellipse (gas envelope) | #9ca3af (Grey) |
| `weight.svg` | Mass from installed upgrades—subtracts from lift | Downward arrow with circle weight | #9ca3af (Grey) |
| `speed.svg` | How fast the ship travels routes | Compass/speedometer dial | #9ca3af (Grey) |
| `range.svg` | Maximum flight distance in segments | Horizontal arrow with origin dot | #9ca3af (Grey) |
| `ceiling.svg` | Maximum altitude—required for mountain routes | Upward arrow with horizontal ceiling line | #9ca3af (Grey) |
| `reliability.svg` | Resistance to hazards during flight | Shield with checkmark | #9ca3af (Grey) |
| `luxury.svg` | Passenger comfort—bonus income on passenger routes | Five-pointed star outline | #9ca3af (Grey) |
...
| `technology.svg` | Tech tiles that unlock upgrade types | Gear/cog with radiating rays | #9ca3af (Grey) |
| `upgrade.svg` | Components installed on the Blueprint | Square with plus sign | #9ca3af (Grey) |
...
| `blueprint.svg` | Your company's ship design template | Grid pattern with circle detail | #9ca3af (Grey) |
| `eye.svg` | Peek action—view hidden information | Eye outline | #9ca3af (Grey) |
| `politics.svg` | Government influence and regulations | Three vertical bars (columns/chart) | #9ca3af (Grey) |
| `gas.svg` | Generic lifting gas (hydrogen or helium) | Gas canister/tank with bands | #9ca3af (Grey) |
