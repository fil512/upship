...
Continuously run `/playtest-phases` in subcontexts until all 4 phases are complete. Each iteration runs in a fresh subcontext to preserve parent context.
...
3. **Game setup:** Run `python -m playtest setup` before starting (uses local server via .upship-config)
...
WHILE true:
  ...
  6. Run /playtest-phases in subcontext (Task tool)
...
When running `/playtest-phases` in a subcontext, use the Task tool with:

```
Task tool:
- subagent_type: "general-purpose"
- prompt: "Execute the /playtest-phases skill. Read plans/playtest-tracking.md first to find the next incomplete phase, then execute that phase's testing workflow using Chrome DevTools MCP tools. Update the tracking file with results. Mark items [x] when verified, document ALL issues found in the 'Issues Found' section."
...
Running /playtest-phases in subcontext...
